...
extern "C"{
    void early_gdt_init();
    void kboot(uint32_t magic, uintptr_t multiboot_info_addr);
...
void higher_half_kernel_init(){
    ...
    uint32_t magic = *((uint32_t*)0x2000);
    uintptr_t multiboot_info_addr = *((uintptr_t*)0x2004); 
    
    // Настройка Пейджинга
    setup_higher_half_mapping();
    early_enable_paging();

    // Второй уровень загрузчика
    kboot(magic, multiboot_info_addr);
}
