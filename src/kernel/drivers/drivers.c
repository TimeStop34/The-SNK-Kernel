...
void driver_registration(struct driver_info* driver, dev_info_t* device){
...
int driver_get(dev_info_t* device, struct driver_info** result){
    /*
    ...
    */
    ...
    for (unsigned int driver_index = 0; driver_index < DRIVER_COUNT; driver_index++){
        ...
        if ((driver->classcode == device->classcode) && (driver->subclass == device->subclass)){
            ...
            int (*init)(dev_info_t*) = (int (*)(dev_info_t*))(driver->init);
