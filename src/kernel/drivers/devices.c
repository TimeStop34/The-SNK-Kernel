...
dev_info_t DEVICES_INFO[256];
unsigned int DEVICE_COUNT;


void device_log(dev_info_t* device) {
    unsigned int dev = 0;

    kprintf("PCI Device: bus=%d dev=%d func=%d class=%x\n",
       device->bus, device->dev, device->func, device->classcode);
    // Тип устройства
       ...
       // x = _get_display_cursor_pos_x(dev);
        ...
        for (;;){
            ...
            if (pci_class_names[pci_class_num].code == device->classcode){
                kprintf(pci_class_names[pci_class_num].name);
                print_flag = 1;
            }
        ...
        for (;;){
            ...
            for (;;){
                ...
                if (device->classcode == subclass_m[pci_subclass_num].classcode && device->subclass == subclass_m[pci_subclass_num].subclass){
                    kprintf("%s \n", subclass_m[pci_subclass_num].name);
                }
...
void device_registration(dev_info_t* device){
