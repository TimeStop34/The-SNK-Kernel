...
static disk_event_callback disk_callback = 0;
...
int disk_register(unsigned int dev_id, struct disk_operations* ops) {
    ...
    // ВЫЗЫВАЕМ КОЛБЭК ПРИ РЕГИСТРАЦИИ НОВОГО ДИСКА
    if (disk_callback) {
        disk_callback(dev_id);
    }
...
void disk_set_event_callback(disk_event_callback callback) {
    disk_callback = callback;
}